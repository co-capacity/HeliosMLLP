...
namespace Helios.MLLP
{
    ...
    /// </summary>
    public class MLLPDecoderOptimized : MLLPDecoderBase
    {
        private int _skipBytes;

        public MLLPDecoderOptimized(byte mllpStartCharacter, byte mllpFirstEndCharacter, byte mllpLastEndCharacter)
            : base(mllpStartCharacter, mllpFirstEndCharacter, mllpLastEndCharacter, 0)
        {
        }


        public MLLPDecoderOptimized(byte mllpStartCharacter, byte mllpFirstEndCharacter, byte mllpLastEndCharacter, int minimiumMessageLength)
        ...
        {
            return new MLLPDecoderOptimized(MLLPStartCharacter, MLLPFirstEndCharacter, MLLPLastEndCharacter);
        }

        #region Static methods

        public static MLLPDecoderOptimized Default
        {
            get
            {
                return new MLLPDecoderOptimized(Convert.ToByte((char)11), Convert.ToByte((char)28), Convert.ToByte((char)13));
            }
        }
